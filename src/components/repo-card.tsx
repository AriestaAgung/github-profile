...
type RepoCardProp = {
    data?: { repos: Repository, item: GithubRepo }[]
};
            ...
            <div>
                {
                    currentData.map((item) =>
                        <Link key={item.item.id} href={item.item.html_url}>
                            <RepoCardView item={item.item} repoData={item.repos}/>
                        </Link>
                    )
                }
...
export function RepoCardView({item, repoData}: { item: GithubRepo, repoData: Repository }) {
    return (
        <>
            <div
    ...
    return (
        <div
            ...
            <div className="flex flex-row justify-between items-center text-theme-dark-color">
                ...
                <div className="">
                    <button
                        className="text-sm px-2 py-1 hover:bg-secondary-dark-color hover:text-main-color hover:border-none hover:border-secondary-dark-color animate-pulse w-20 h-7 bg-slate-400 rounded-lg"></button>
                </div>
            </div>
        </div>
    );
